...
import { showAnalysisSummary, showCompletionSummary } from './ui/summaryModal';
...
export async function startBugQueue(editor: vscode.TextEditor) {
    ...
    await vscode.window.withProgress({
        ...
        // Start backend logic analysis concurrently
        const backendPromise = analyzeAllAPI(document, diagnostics);
        ...
        if (!backendFailureDetected) {
            progress.report({ message: "Running AI Logic Scan..." });
            try {
                const backendFindings = await backendPromise;
                if (backendFindings) {
                    let newFindings = backendFindings.filter(f => f.severity === 'Tier 1' || !queueState.skippedBugs.has(f.concept));
                    
                    const existingConcepts = new Set(localMissions.map(b => b.description));
                    const uniqueNew = newFindings.filter(f => !existingConcepts.has(f.concept));
                    
                    if (uniqueNew.length > 0) {
                        localMissions.push(...uniqueNew.map(f => mapUnifiedToMission(f, document)));
                    }
                }
            } catch (e) {
    ...
    let runtimeCount = localMissions.filter(m => m.tier === 2).length;
    let logicCount = localMissions.filter(m => m.tier === 3).length;
    ...
    queueState.stats = { syntax: syntaxCount, runtime: runtimeCount, logic: logicCount };
    ...
    // Show Summary Popup
    const startFixing = await showAnalysisSummary({
        total: queueState.totalBugs,
...
export async function processCurrentBug() {
    ...
    if (queueState.currentIndex >= queueState.bugs.length) {
        ...
        const action = await showCompletionSummary({
            syntax: queueState.fixedStats.syntax,
            runtime: queueState.fixedStats.runtime,
            logic: queueState.fixedStats.logic
        });
